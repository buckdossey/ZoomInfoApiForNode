...
export default class ZoomInfoContactEnrich extends ZoomInfoApiBase {
  inputFields: object[];
  outputFields: string[];
  constructor() {
  ...
  async enrichContact() {
    try {
      ...
      const response = await this.post('/enrich/contact', params);

      return response?.data?.result[0]?.data[0];
    } catch (error: any) {
      console.error('Error during ZoomInfoContactEnrich.enrichContact:', error.response?.data || error.message);
      //throw error;
    }
  }
}
